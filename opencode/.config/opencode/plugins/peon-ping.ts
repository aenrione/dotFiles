...
const TMUX_STATE_DIR = path.join(os.homedir(), ".openpeon", "tmux-state")
const PEON_TMUX_UPDATE = path.join(os.homedir(), ".local", "bin", "peon-tmux-update")
...
function loadState(): PeonState {
  try {
    ...
    return { last_played: {}, session_packs: {}, subagent_sessions: {} }
  }
}
...
/** Get the current tmux pane ID (e.g., %42). */
function getTmuxPaneId(): string {
  return process.env.TMUX_PANE || ""
}

/**
 * Write tmux state and update visuals (window name, pane border, status bar).
 * Calls peon-tmux-update as an async subprocess for fire-and-forget updates.
 */
function writeTmuxState(
  category: string,
  project: string,
  status: string,
): void {
  ...
  // Check if peon-tmux-update script exists
  if (!fs.existsSync(PEON_TMUX_UPDATE)) return
  ...
  try {
    ...
    // Non-critical: tmux updates are best-effort
  }
}

/**
 * Clear tmux state for the current pane and restore defaults.
 */
function clearTmuxState(): void {
  if (!isInsideTmux()) return
  ...
  try {
    const proc = Bun.spawn(
      [PEON_TMUX_UPDATE, "--pane", paneId, "--clear"],
      {
        stdout: "ignore",
        stderr: "ignore",
        env: { ...process.env },
...
export const PeonPingPlugin: Plugin = async ({ directory }) => {
  ...
  function pruneSubagentSessions(nowMs: number = Date.now()): void {
    const cutoff = nowMs - 300000
    const subs = state.subagent_sessions
    let changed = false
    for (const [sid, ts] of Object.entries(subs)) {
      if (ts < cutoff) {
        delete subs[sid]
        changed = true
      }
  ...
  async function emitCESP(
    ...
    // tmux visual updates (always, even when paused)
    writeTmuxState(category, projectName, status)
  ...
  return {
    event: async ({ event }) => {
      switch (event.type) {
        // Task complete
        case "session.idle": {
          if (config.suppress_subagent_complete) {
            const subSessionId = event.properties?.sessionID
            if (subSessionId && state.subagent_sessions[subSessionId]) {
              break
            }
          }
        ...
        case "session.created": {
          if (config.suppress_subagent_complete) {
            ...
            if (info?.parentID && info.id) {
              state.subagent_sessions[info.id] = Date.now()
        ...
        case "session.deleted": {
          if (config.suppress_subagent_complete) {
            const info = event.properties?.info
            if (info?.id && state.subagent_sessions[info.id]) {
              delete state.subagent_sessions[info.id]
              saveState(state)
            }
          }
          // Clear tmux state when session ends
          clearTmuxState()
          break
        }
